# Limiting network access to Metal APIs

Cloudrock Helm allows limiting network access to Metal API endpoints - i.e. `/api/`, `/api-auth/`, `/admin/` - based on whitelisting the subnets from where access is allowed. To define a list of allowed subnets in CIDR format for the all the API endpoint, please use `ingress.whitelistSourceRange` option in `values.yaml`. Example:
...
Given this value, only IPs from `192.168.22.0/24` subnet are able to access Cloudrock Metal APIs.
...
In case of multiple subnets/IPs, a comma separated list can be used as a value. E.g.: `192.168.22.1/32,192.168.21.0/24`. This works for both options.
